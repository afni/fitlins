...
def test_get_intent_info_for_subvol():
    brick = get_reml_bucket_test_data()
    intent_info = get_intent_info_for_subvol(brick, 1)
    assert intent_info == ("t test", (420,))


def test_get_intent_info():
    ...
    assert expected == get_intent_info(img)

    img.header.info.pop("BRICK_STATSYM")
    assert get_intent_info(img) == [("none", ()), ("none", ()), ("none", ())]

    img.header.info["BRICK_STATSYM"] = "Zscore();none;none"
    assert get_intent_info(img) == [("z score", ()), ("none", ()), ("none", ())]
